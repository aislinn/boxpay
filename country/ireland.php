<?php $subtitle = "Ireland";
include($_SERVER['DOCUMENT_ROOT'].'/includes/head.php');		
include($_SERVER['DOCUMENT_ROOT'].'/includes/header.php')?>	
	
		
	<div class="row primary-row">	
  		
  		
  		
  		<section role="main">
  			
  			
  			<div class="row primary-container">
  				<div class="eleven columns offset-by-one">
  					<div class="row">
  					
  						<div class="six columns">
  							<h1>Countries &amp; Price Points</h1>
  							<p>Sed nec diam eu diam mattis viverra. Nulla fringilla, orci ac et euismod semper, magna diam porttitor mauris, quis sollicitudin sapien justo in libero. Vestibulum mollis mauris enim. Morbi.</p>
  							
  							  							
  						</div>
  						
  						
  						<div class="six columns countries-map"></div>

  					
  					</div>
  				</div>
  				
  				
  				<div class="twelve columns divider"></div>
  				
  				
  				<div class="twelve columns">
  					<div class="row">
  					
  						
  						<div class="three columns end push-eight">
  						
  						
  						<div id="accordion" class="region-accordion">
  							<h6><a href="#">Western Europe</a></h6>
  							<div>
  								<ul class="no-bullet">
  									<li><a href="austria.html">Austria</a></li>
  									<li><a href="belgium.html">Belgium</a></li>
  									<li><a href="france.html">France</a></li>
  									<li><a href="germany.html">Germany</a></li>
  									<li><a href="greece.html">Greece</a></li>
  									<li><a href="ireland.html">Ireland</a></li>
  									<li><a href="netherlands.html">Netherlands</a></li>
  									<li><a href="portugal.html">Portugal</a></li>
  									<li><a href="switzerland.html">Switzerland</a></li>
  									<li><a href="united-kingdom.html">United Kingdom</a></li>
  								</ul>
  							</div>
  							
  							<h6><a href="#">Northern Europe</a></h6>
  							<div>
  								<ul class="no-bullet">
  									<li><a href="denmark.html">Denmark</a></li>
  									<li><a href="estonia.html">Estonia</a></li>
  									<li><a href="finland.html">Finland</a></li>
  									<li><a href="latvia.html">Latvia</a></li>
  									<li><a href="lithunia.html">Lithuania</a></li>
  									<li><a href="norway.html">Norway</a></li>
  									<li><a href="sweden.html">Sweden</a></li>
  									
  									
  								</ul>  							
  							</div>
  							
  							<h6><a href="#">Eastern &amp; Central Europe</a></h6>
  							<div>
  								
  								<ul class="no-bullet">
  									<li><a href="bulgaria.html">Bulgaria</a></li>
  									<li><a href="croatia.html">Croatia</a></li>
  									<li><a href="czech-republic.html">Czech Republic</a></li>
  									<li><a href="hungray.html">Hungary</a></li>
  									<li><a href="macedonia.html">Macedonia</a></li>
  									<li><a href="poland.html">Poland</a></li>
  									<li><a href="romania.html">Romania</a></li>
  									<li><a href="serbia.html">Serbia</a></li>
  									<li><a href="slovakia.html">Slovakia</a></li>
  									<li><a href="slovenia.html">Slovenia</a></li>
  							
  								</ul>
  							</div>
  							<h6><a href="#">North America</a></h6>
  							
  							<div>
  								<ul class="no-bullet">
  									<li><a href="canada.html">Canada</a></li>
  									<li><a href="united-states.html">United States</a></li>
  									<li><a href="mexico.html">Mexico</a></li>
  									
  								</ul>
  							</div>
  							
  							<h6><a href="#">South America</a></h6>
  							<div>
  								<ul class="no-bullet">
  									<li><a href="argentina.html">Argentina</a></li>
  									<li><a href="brazil.html">Brazil</a></li>
  									<li><a href="colombia.html">Colombia</a></li>
  
  								</ul>  							
  							</div>
  							
  							<h6><a href="#">Asia Pacific</a></h6>
  							<div>
  								<ul class="no-bullet">
  									<li><a href="australia.html">Australia</a></li>
  									<li><a href="china.html">China</a></li>
  									<li><a href="hong-kong.html">Hong Kong</a></li>
  									<li><a href="india.html">India</a></li>
  									<li><a href="indonesia.html">Indonesia</a></li>
  									<li><a href="malaysia.html">Malaysia</a></li>
  									<li><a href="philippines.html">Philippines</a></li>
  									<li><a href="singapore.html">Singapore</a></li>
  									<li><a href="thailand.html">Thailand</a></li>
  									<li><a href="vietnam.html">Vietnam</a></li>
  								</ul>  							
  							</div>
  							
  							
  							<h6><a href="#">Middle East &amp; Africa</a></h6>
  							<div>
  								<ul class="no-bullet">
  									<li><a href="israel.html">Israel</a></li>
  									<li><a href="south-africa.html">South Africa</a></li>
  									<li><a href="turkey.html">Turkey</a></li>
  								</ul>  							
  							</div>
  							
  						</div>
  						
  						
  						</div><!-- End demo -->
  						
  						<div class="six columns pull-five">
  						<h2>Mobile payments in Ireland</h2>
  						
  						<h6>Available Price Points</h6>
  						<p>5, 10, 15, 20, 25, 30, 35</p>
  						
  						
  						<h6>Billing Type</h6>
  						<p>PSMS / MT Billed</p>
  						
  						<h6>Restrictions</h6>
  						<p>In hac habitasse platea dictumst. 
  						Nam pulvinar, odio sed rhoncus em suscipit, sem diam ultrices mauris
  						Reu consequat purus metus.</p>
  						
  						<h6>Country Information</h6>
  						<p>Sed nec diam eu diam mattis viverra. Nulla fringilla, orci ac et euismod semper, magna diam porttitor mauris, quis sollicitudin sapien justo in libero. Vestibulum mollis mauris enim. Morbi.</p>
  						
  						
  						</div>
  						
  						
  						
  						</div>
  					
  					
  					</div>
  				</div>
  				
  				
  				
  				
  		
  			
  		
  		</section>
  		
  		
  		
  		
  		<?php include($_SERVER['DOCUMENT_ROOT'].'/includes/sidebar.php')?>
		
	</div><!-- container -->
			
 
<?php include($_SERVER['DOCUMENT_ROOT'].'/includes/footer.php')?>